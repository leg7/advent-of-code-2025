...
func main() {
	...
	pairsToEvaluate := 1000

	for i, segment := range segments {
		if i == pairsToEvaluate {
			break
		}
	...
	for k, v := range circuitGraph.AdjacencyList {
		fmt.Println(k, v)
	}
	...
	assert.True(len(circuitSizes) >= 3, "We must have atleaste 3 circuits to compute the result")
