...
func main() {
	...
	for scanner.Scan() {
		...
		assert.NoError(err)
		assert.True(!seen[ingredient], "Duplicate ingredient")
		seen[ingredient] = true

		position, found := slices.BinarySearchFunc(rangesCompressed, ingredient, func (r Range, i int64) int {
			return cmp.Compare(r.lb, i)
		})

		if found {
			freshIngredientCount++
		} else if position == 0 && rangesCompressed[1].lb == ingredient {
			freshIngredientCount++
		} else if position != 0 && ingredient <= rangesCompressed[position-1].ub {
			freshIngredientCount++
		}
	}
